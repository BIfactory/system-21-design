...
hdr=$(node -e "const m=require('./src/previews/meta.json');console.log(JSON.stringify({format:4,namespace:'S21',components:Object.keys(m).filter(k=>m[k][3]!=='page').map(name=>({name}))}))")
...
for n in $(node -e "console.log(Object.keys(require('./src/previews/meta.json')).join(' '))"); do
  ...
  N="$n" node -e '
    const fs=require("fs"), n=process.env.N; const [g,h,w,pg]=require("./src/previews/meta.json")[n];
    ...
    fs.writeFileSync("components/"+n+"/preview.html","<!-- @dsCard group=\""+g+"\" height="+h+(w?" width="+w:"")+(pg?" page":"")+" -->\n<div id=\"root\"></div>\n<script>"+js+"</script>\n");'
done
rm -f .tmp.js
echo "Build OK"
